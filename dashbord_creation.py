...
# Main dashboard
st.title("Dashboard for Annonces for filtrage")
...
if "surface_area" in filtered_df.columns and "price" in filtered_df.columns:
    ...
    st.warning("Surface area or price data is not available.")
st.title("Dashboard for Sorting data")


# Sorting
st.sidebar.subheader("Sort Data")
sort_by = st.sidebar.selectbox("Sort By", options=["None", "Price", "Number of Rooms", "Number of Bathrooms"])
sort_order = st.sidebar.radio("Sort Order", options=["Ascending", "Descending"])

# Apply Sorting
if sort_by != "None":
    if sort_order == "Ascending":
        filtered_df = filtered_df.sort_values(by=sort_by.lower().replace(" ", "_"), ascending=True)
    else:
        filtered_df = filtered_df.sort_values(by=sort_by.lower().replace(" ", "_"), ascending=False)

# Pagination
st.sidebar.subheader("Pagination")
page_size = st.sidebar.slider("Number of rows per page", 5, 50, 10)
...
current_page = st.sidebar.slider("Page", 1, total_pages, 1)
...
st.subheader("Filtered Annonces Table par sort")
st.dataframe(paginated_df)



# Search bar for specific keyword
search_query = st.sidebar.text_input("Search for annonces")
...
st.subheader("Filtered Annonces Table par la recherche")
st.dataframe(filtered_df)
...
# Calculate total number of annonces
total_annonces = len(filtered_df)

# Calculate total number of cities represented
total_cities = filtered_df['city_id'].nunique()

# Display the statistics
st.write(f"**Average Price**: ${average_price:,.2f}")
st.write(f"**Median Price**: ${median_price:,.2f}")
st.write(f"**Total Number of Announcements**: {total_annonces}")
st.write(f"**Number of Cities Represented**: {total_cities}")
